function [A,F] = build_helm_system (m_x, m_y, A_matrix, F_matrix, upper_bound, bottom_bound)                   % AU = F
    ...
    for i = 2 : m_y-1
        for j = 1 : m_x
            ...
            elseif k <= count_variables && k >= count_variables - m_x     % Condição de contorno para y = m_y - 1
            ...
            elseif k % m_x == 1                                           % Condição de contorno para x = -1
                A_matrix(k,k+1) = 2/h_x/h_x;
                A_matrix(k,k-m_x) = 1/h_y/h_y;
                A_matrix(k,k+m_x) = 1/h_y/h_y;
            elseif k % m_x == 0                                           % Condição de contorno para x = 1
                A_matrix(k,k-1) = 2/h_x/h_x;
                A_matrix(k,k-m_x) = 1/h_y/h_y;
                A_matrix(k,k+m_x) = 1/h_y/h_y;
            else
